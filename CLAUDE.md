...
Qelvora is a Laravel 13 + Inertia v3 + Vue 3 (TypeScript) application built on the Laravel Vue starter kit, extended with **team-based multi-tenancy**. Auth is handled by Laravel Fortify (login, registration, email verification, password reset, 2FA/TOTP, recovery codes). Routes are typed end-to-end via Laravel Wayfinder.
